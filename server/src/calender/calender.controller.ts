...
async endEvent(@_OAuth2Client() client: OAuth2Client, @Body('eventId') eventId: string): Promise<ApiResponse<EventUpdateResponse>> { 
...
const updatedEvent = await this.calenderService.endEvent(client, eventId); 

return createResponse(updatedEvent, 'Meeting has been ended'); 
