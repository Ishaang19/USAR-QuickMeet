...
// Find the organizer from attendees 

const organizerAttendee = event.attendees?.find((attendee) => attendee.organizer === true); 

if (organizerAttendee) { 

event.organizer = { email: organizerAttendee.email }; 

} else { 

// Fallback to random email if no organizer found in attendees 
...
const newEvent = { ...event, id: randomId }; 
